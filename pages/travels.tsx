...
export default function TravelsPage() {
  const [photosAndTravelPosts, setPhotosAndTravelPosts] = useState<
    PhotoAndTravelPost[]
  >([]);
  ...
  const fetchData = useCallback(
    async (selectedPage: number) => {
      ...
      const results = await getPhotosAndTravelPosts({
        skipPosts,
        skipPhotos,
      });
      let countPhotos = 0;
      ...
      setTotalPage(
        Math.ceil((results.postsCount + results.photosCount / 2) / 6),
      );
      setIsLoading(false);
    },
  ...
  return (
    <div className="w-full h-full -mt-12 pt-12 flex flex-col">
              ...
              <>
                {type == PhotoAndPost.post ? (
                  <PostCard
                    ...
                    date={
                      new Date((data as Post).displayedDate || data.createdAt)
                    }
                    key={data.id}
      ...
      {photosAndTravelPosts.length > 0 && (
        <Pagination pageNumber={totalPage} callBack={fetchData} />
      )}
      {!isLoading && photosAndTravelPosts.length === 0 && (
        <h2 className="pt-10 text-xl font-finlandica-500">
          De prochains articles seront bientôt disponibles !
        </h2>
      )}
    </div>
  );
