  ...
  return (
    <div className={classes + " shadow-lg p-3 relative bg-white w-full" + ( index % 2 === 0 ? " card" : " text-center green-border")}>
      <div className='text-left mb-2'>
        <h4 className='transition duration-100 cursor-pointer hover:text-violet-900 text-xl font-semibold'>
          <Link href={`/post/${url}`}>{title}</Link>
        </h4>
        <span className='font-medium text-gray-700 text-sm space-x-1'>{theme}</span>
      </div>
      ...
      <div className='text-center flex flex-col px-2 items-center bottom-2 absolute w-11/12 -translate-x-2/4 left-2/4'>
        ...
        <div className='flex justify-between w-full'>
          <span className='font-medium text-gray-700 text-sm space-x-1'>
