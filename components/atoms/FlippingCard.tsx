...
export default function FlippingCard({
  ...
  return (
    <div
      className={
        "flex h-64 items-center justify-center relative" +
        (index % 3 === 0 ? " col-start-1" : "")
      }
      onClick={() => setFlipped((state) => !state)}
    >
      <a.div
        className="absolute overflow-hidden shadow-md pb-64 mb-5 cursor-pointer will-change-transform w-full top-0"
        style={{ opacity: opacity.to((o) => 1 - o), transform }}
      >
        <img
          src={imageURL}
          alt={title}
          className="object-top absolute h-64 w-full object-cover shadow-lg rounded-lg"
        ></img>
      </a.div>
      <a.div
