...
export default function PostDetail({
  ...
  const getContentFragment = (
    ...
    switch (type) {
      case "heading-three":
        return (
          <h3 key={index} className={`text-xl font-semibold mb-4 ${className}`}>
      ...
      case "heading-four":
        return (
          <h4 key={index} className={`text-md font-semibold mb-4 ${className}`}>
            {modifiedText.map((item: any, i: number) => (
              <React.Fragment key={i}>{item}</React.Fragment>
            ))}
          </h4>
      ...
      case "bulleted-list":
        return (
          <ul
            key={index}
            className={`pl-5 list-disc text-lg mb-4 ${className}`}
          >
            {obj.children.map((typeObj: any, index: any) => {
              const children2 = typeObj.children.map(
