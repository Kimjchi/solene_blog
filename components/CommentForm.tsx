...
export default function CommentForm({ postID }: { postID: string }) {
  ...
  return (
    <div className="mt-3 md:w-5/6">
      <h3 className="ml-3 font-finlandica-500 text-xl mb-5">
        Zone de commentaires
      </h3>
      <div className="flex flex-col w-full h-full yellow-border shadow-lg p-3 relative bg-white rounded-lg">
        <h4 className="ml-3 font-finlandica-500">
          {numberOfComments} commentaire(s)
        </h4>
        <div className="mb-10">
          {comments.map((comment, index) => (
            <div className="space-y-2" key={comment.id}>
              <div className="m-2 bg-slate-50 flex flex-col pl-3">
                <div className="flex w-full justify-between">
                  ...
                  <span
                    ...
                    onClick={() => {
                      setIsReplying(
                        !isReplying || comment.id !== commentReplying?.id,
                      );
              ...
              {isReplying && commentReplying?.id === comment.id && (
                <CommentInputGroup
                  postID={postID}
              ...
              <div className="pl-3">
                {comment.childComments.map((childComment) => (
                  <div className="flex" key={childComment.id}>
                    ...
                    <div
                      className="mx-2 bg-slate-50 flex flex-col pl-5 w-full"
                      key={childComment.id}
                    >
                      <div className="flex items-center w-full">
                        <h4 className="text-lg bold mr-2">
                          {childComment.name}
                        </h4>{" "}
                        <span>{getHowLongAgo(childComment.createdAt)}</span>
                      </div>
...
function CommentInputGroup({
  ...
  const handleSubmit = useCallback(async () => {
    ...
    (commentReplying
      ? submitChildComment({
          ...
          email,
          comment,
          parentCommentID: commentReplying.id,
          captcha: token,
        })
      : submitComment({
          name,
  ...
  return (
    <animated.div
      className=" w-full h-full grid grid-cols-5 bg-slate-50 p-3"
      style={props}
    >
      <div className="flex flex-col space-y-2 ml-3 col-span-4">
        <textarea
          ...
          className={`focus:outline-none ${
            commentErr ? "border-rose-500 border" : "yellow-border"
          }`}
          value={comment}
          onChange={(e) => setComment(e.target.value)}
        />
        {commentErr && (
          <span className="text-rose-500 text-sm mt-0">
            Le commentaire ne peut contenir de lien.
          </span>
        )}
        <input
          placeholder=" Nom*"
          type="text"
          required
          className="yellow-border w-1/4 focus:outline-none"
          value={name}
        ...
        <input
          ...
          className={` w-1/4 focus:outline-none ${
            emailErr ? "border-rose-500 border" : "yellow-border"
          }`}
          value={email}
      ...
      <div className="relative">
        <input
          ...
          disabled={isDisabled}
          value="Envoyer"
          className={`w-20 absolute bottom-0 right-3 ${
            !isDisabled ? "yellow cursor-pointer" : ""
          }`}
        />
      </div>
    </animated.div>
