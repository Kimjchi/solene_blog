...
const NavigationBar = () => {
  ...
  return (
    <>
      <div className='container mx-auto w-full justify-between pt-8 md:flex hidden'>
        {
          categories.map((category: any) => (<Link href={category.slug} key={category.slug}><span className='cursor-pointer font-bold text-sm lg:text-base'>{category.name}</span></Link>))
        }
      </div>
      <div className='md:hidden'>
        <Menu right isOpen={ isOpen } onOpen={() => setIsOpen(true)} onClose={ () => setIsOpen(false) }>
          {
            categories.map((category: any) => (<Link href={category.slug} key={category.slug}><span className='cursor-pointer font-bold bm-item' onClick={() => setIsOpen(false)}>{category.name}</span></Link>))
          }
          <div className='introduction card w-full shadow-lg rounded-lg py-1 mt-5'>
