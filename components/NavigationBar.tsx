...
const NavigationBar = () => {
  ...
  useEffect(() => {
    getCategories().then((newCategories: any) =>
      setCategories([
        { name: "Accueil", slug: "" },
        ...newCategories,
        { name: "À propos", slug: "about" },
    ...
    <>
      <div className="container mx-auto w-full justify-between px-5 pt-8 md:flex hidden font-finlandica-500">
        {categories.map((category: any) => (
          <Link replace href={`/${category.slug}`} key={category.slug}>
            <span className="cursor-pointer lg:text-base hover-yellow">
              {category.name}
            </span>
          </Link>
        ))}
        ...
        >
          ...
          >
            ...
            <input
              ...
              type="text"
              required
              className="focus:outline-none yellow ml-1"
              value={keyword}
          ...
          {categories.map((category: any) => (
            <Link replace href={`/${category.slug}`} key={category.slug}>
              <span
                className="cursor-pointer bm-item font-finlandica"
          ...
          <div className="introduction text-center w-full py-1 mt-5">
            <div className="relative overflow-hidden pb-40 mb-4">
              <img
                ...
                className="object-top absolute h-40 w-full object-contain rounded-lg"
              ></img>
            </div>
            <h4 className="mx-auto font-finlandica-500 text-center">
              Introduction
            </h4>
            <p className="mx-auto text-center px-2">
              ...
              <br />
              Ici on parle de littérature et de cinéma, d’arts et d’histoire et
              de bien d’autres choses ; bref, de la Corée dans tous ses états.
            </p>
          </div>
              ...
              {tags.map(({ name, slug }, index: number) => (
                <Link href={`/tag/${slug}`} key={slug}>
                  <span
                    key={slug}
                    className={
                      "px-3 py-1 cursor-pointer border hover:bg-gray-100" +
                      (index === 0 ? " mt-2 ml-2" : "")
                    }
                    onClick={() => setIsOpen(false)}
                  >
